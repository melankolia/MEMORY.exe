  ...
  <div class="fixed bottom-4 right-4 flex flex-col-reverse gap-2 z-50">
    <TransitionGroup name="slide-fade">
      <div
        ...
        :key="toast.id"
        class="relative overflow-hidden bg-purple-900/80 backdrop-blur-sm text-white px-2.5 py-4 rounded-xl shadow-2xl flex items-center space-x-3 border border-purple-700/50 cursor-pointer"
        @click="handleToastClick(toast)"
      >
        <!-- Glow Effect -->
        <div
          class="absolute inset-0 bg-gradient-to-r from-purple-500/20 to-pink-500/20 blur-xl"
        ></div>
        ...
        <div class="relative flex items-center space-x-3">
          ...
          >
            <svg
              ...
              class="h-4 w-4 text-fuchsia-300 hover:text-white"
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
            >
        ...
        >
          <div
            class="absolute inset-0 bg-gradient-to-r from-white/20 to-transparent animate-shimmer"
          ></div>
        </div>
      </div>
    </TransitionGroup>
...
<script setup>
  ...
  const handleToastClick = async (toast) => {
    if (toast.message.startsWith("Tx:")) {
      try {
        await navigator.clipboard.writeText(props.fullHash);
        // Add a new toast for copy confirmation
        addToast("Transaction hash copied to clipboard!");
      } catch (err) {
